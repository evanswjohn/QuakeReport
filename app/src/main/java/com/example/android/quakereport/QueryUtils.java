...
public final class QueryUtils {
     ...
     * multiple earthquakes as defined by the HTTP query.
     */
    public static List<Earthquake> fetchEarthquakeData(String requestUrl) {
        ...
        // Return the {@link Earthquake} object
        return earthquakes;
    }

    /**
     * Return new URL object frm given string URL
     * */
    ...
    public static List<Earthquake> extractEarthquakes(String earthquakeJSON) {
        ...
        try {
            ...
            // Get the instance of JSONArray that contains JSONObjects "features"
            JSONArray jsonArrayFeatures = jsonRootObject.getJSONArray("features");

            // Iterate over the JSONArray and get the JSONObject "properties"
            for(int i = 0; i < jsonArrayFeatures.length(); i++) {
                JSONObject earthquake = jsonArrayFeatures.optJSONObject(i);
                JSONObject properties = earthquake.optJSONObject("properties");
                // get the magnitude, place and time from properties
                double mag = properties.getDouble("mag");
                String place = properties.getString("place");
                long time = properties.getLong("time");

                // get the value for teh "url" key
                String url = properties.getString("url");

                /**
                 * Create a new {@ling Earthquake} object with the magnitude, location,
